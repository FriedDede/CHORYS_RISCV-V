...
// uncomment when executing on the LAMP CPU
#define _LAMP
// uncomment to generate random inputs instead of using default generated inputs
//#define _RANDOM_INPUTS
...
//        x = x + dx;
//        diff = val - (x * x);
//        if ((diff >= 0.0f? diff : -diff) <= min_tol)
//          flag = 1;
//      }
//    }
//  }
...
int rand(void) // RAND_MAX assumed to be 32767
{
  ...
  return (unsigned int)(next) % (32768*32768);
  //return (unsigned int)(next/65536) % (32768); // original from glibc
}
...
float randFloat()
{
  float val, fract;
  int tmp;
  fract = ((float) rand() / (float)rand()+1);
  tmp = (int) fract;
  fract = fract-(float)tmp;
  val = (float) (rand() % d_range) + fract;
  return val;
}
#endif
